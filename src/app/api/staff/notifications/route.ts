...
export async function GET(request: Request) {
    ...
    const { status, data, error } = await fetchFromBE(`/staff/notifications?${queryString}`, 'GET');
    if (error) return NextResponse.json({ error }, { status });
    return NextResponse.json(data, { status });
}

export async function PATCH() {
    const { status, data, error } = await fetchFromBE('/staff/notifications/read-all', 'PATCH');
    if (error) return NextResponse.json({ error }, { status });
    return NextResponse.json(data, { status });
}